...
3. **Write a test for it.** A rule test is one YAML file under
   ...
   `expect: []`; failures print ready-to-paste entries, and you adopt
   the intended ones. An empty list that stays empty asserts complete
   ...
   rule: "domain/rule-is-sole-aggregate"
   files:
...
vocabulary/terms-carry-definitions:
  description: "Every recorded term carries a definition."
  on: vocabulary
  uses: require-defined-terms
```

Patterns carry their own extensions: extend `arclint/vertical@0.1.0`
