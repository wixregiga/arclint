...
export interface DomainAssertion { key: string; on: string; statement: string }
export interface Concept { id: string; name: string; kind: ConceptKind; contextId: string; definition: string; position: [number, number, number]; invariants: string[]; identity?: string; ownerId?: string; aliases?: string[]; assertions?: DomainAssertion[]; raisedBy?: string }
...
/** Legacy type name retained for the v1 storage adapter. */
export type Baseline = ModelSnapshot;
