...
app.innerHTML = `
  <main class="atlas-world" aria-label="Domain atlas">
    ...
    <div class="map-caption"><span class="caption-rule"></span><p id="level-hint">The recorded contexts and their relationships.<br /><small>Drag to explore · Double-click a context to enter</small></p></div>
...
function positionManuscript() {
  ...
  const y = anchor ? Math.max(210, Math.min(window.innerHeight - 410, anchor.y - 155)) : Math.max(210, window.innerHeight * .24);
...
function renderLocation() {
  ...
  $('#level-hint').innerHTML = aggregate ? `Within ${escape(aggregate.name)}.<br /><small>Its recorded members and relationships</small>` : context ? `Inside ${escape(context.name)}.<br /><small>A shared language, within a boundary</small>` : 'The recorded contexts and their relationships.<br /><small>Drag to explore · Double-click a context to enter</small>';
}
...
function render() {
  workbench?.update(project, selectedId, scopeId, mode, baseline);
...
workbench = createWorkbench($('.atlas-world'), { select: id => { mode = 'domain'; select(id, true); }, replace: next => { selectedId = null; mode = 'domain'; search = ''; ($('#search') as HTMLInputElement).value = ''; commit(next, undefined, true); scene.overview(); }, create: () => conceptDialog(), notify });
