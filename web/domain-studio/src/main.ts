...
app.innerHTML = `
  <main class="atlas-world" aria-label="ArcLint places" data-lens="meaning">
    <div id="scene" data-testid="scene" aria-label="Interactive domain landscape"></div>
    <header class="place-header"><button id="home-world" class="place-brand" aria-label="Whole domain">arclint<span>places</span></button><h1 id="project-name"></h1><div class="place-header-actions"><button id="open-index" aria-label="Find a concept">${icon('search', 18)} Find</button><button id="tools-toggle" aria-label="Tools">Tools ${icon('plus', 16)}</button></div></header>
    ...
    <button id="keeper-action" class="keeper-action" aria-label="Ask the keeper"><span id="keeper-name">Onyx</span><span id="keeper-prompt">Choose a place to begin</span></button>
    ...
    <dialog id="tools-drawer" class="tools-drawer" aria-label="Tools"><header><span>WORK WITH THIS PLACE</span><button id="close-tools" aria-label="Close tools">${icon('x',22)}</button></header><h2>Tools</h2><div class="tools-body">
      <section class="tool-group"><h3>Shape the model</h3><button id="add-context">${icon('hexagon',16)} Add context</button><button id="add-concept">${icon('plus',16)} Add concept</button><button id="connect" aria-label="Connect concepts">${icon('waypoints',16)} Connect concepts</button><button id="edit-project">${icon('pencil',16)} Domain details</button></section>
      <section class="tool-group workspace-access"><button id="workspace-menu-toggle" aria-label="Workspace menu" aria-expanded="false">Files & workspace ${icon('chevron-right',16)}</button><div id="workspace-menu" hidden><button id="new-domain">New domain</button><button id="import">Import</button><button id="export">Export</button><button id="load-example-menu">Load example domain</button><button id="menu-domain-details">Domain details</button></div></section>
      <section class="tool-group"><h3>Review</h3><button data-mode="patterns" aria-label="Patterns">Patterns</button><button data-mode="baseline" aria-label="Baseline">Compare baseline</button><button id="field-guide" aria-label="Field guide">Recorded language guide</button></section>
  ...
  <dialog id="navigator" class="index-sheet" aria-label="Domain index"><div class="index-heading"><div><span>EVERY PLACE REMAINS REACHABLE</span><h2>Find</h2></div><button id="close-index" class="icon-button" aria-label="Close index">${icon('x', 20)}</button></div><label class="index-search">${icon('search', 20)}<input id="search" placeholder="A concept, context, or meaning…" aria-label="Find a concept" autocomplete="off" /></label><div id="model-tree" data-testid="model-tree"></div><p class="index-footer">Select a name to go there. <span id="context-count"></span> contexts.</p></dialog>
...
function undo() { const last = history.pop(); if (!last) return; future.push(snapshot()); ({ project, baseline, patterns } = last); selectedId = null; editorOpen = false; viewPage = 0; persist(); render(); scene.overview(); notify('Change undone'); }
function redo() { const next = future.pop(); if (!next) return; history.push(snapshot()); ({ project, baseline, patterns } = next); selectedId = null; editorOpen = false; viewPage = 0; persist(); render(); scene.overview(); notify('Change restored'); }
...
function renderLocation() {
  ...
  $('#keeper-action').dataset.keeper = keeper.name;
  $('#keeper-action').setAttribute('aria-label', `${keeper.name}: ${keeper.prompt}`);
  $('#open-editor').hidden = view.level === 'world' || lens === 'governance';
  $('#primary-action').hidden = view.level !== 'world' && lens === 'meaning';
  $('#primary-action').textContent = lens === 'governance' ? 'Inspect governing Rules ↗' : 'Add a context +';
...
function bindInspector() {
  $('#clear-selection')?.addEventListener('click', () => { editorOpen = false; mode = 'domain'; render(); });
  $('#close-sheet')?.addEventListener('click', () => { editorOpen = false; if (mode !== 'baseline') mode = 'domain'; render(); });
  ...
  document.querySelectorAll<HTMLElement>('[data-change]').forEach(b => b.onclick = () => { const id = b.dataset.change!; if (![...project.contexts, ...project.concepts, ...project.relationships].some(item => item.id === id)) { notify('Removed from the current model. Download the baseline to keep its earlier record.'); return; } select(id, true); editorOpen = false; mode = 'baseline'; render(); });
...
function showDialog(title: string, contents: string, submitLabel: string, onSubmit: (data: FormData) => void) {
  ...
  modal.innerHTML = `<form id="dialog-form"><div class="dialog-heading"><div><div class="eyebrow">ARCLINT PLACES</div><h2>${escape(title)}</h2></div><button type="button" class="icon-button" id="close-dialog" aria-label="Close dialog">${icon('x', 20)}</button></div><div class="dialog-content">${contents}<p class="form-error" role="alert"></p></div><div class="dialog-actions"><button class="quiet-button" type="button" id="cancel-dialog">Cancel</button><button class="primary" type="submit">${escape(submitLabel)}</button></div></form>`;
...
function conceptDialog(contextId?: string) {
  ...
  showDialog('Add concept', `${field('Name', 'name')}${field('Definition', 'definition', '', true, false)}<div class="field-pair"><label class="field"><span>Kind</span><select name="kind">${kindOptions()}</select></label><label class="field"><span>Context</span><select name="contextId">${contextOptions(selectedContext)}</select></label></div>${metadataFields()}${field('Invariants', 'invariants', '', true, false)}<p class="muted small">Keep a concept unclassified until its identity and responsibilities are understood.</p>`, 'Create concept', data => {
    ...
    next.concepts.push({ id, name: String(data.get('name')).trim(), definition: String(data.get('definition')).trim(), kind: String(data.get('kind')) as ConceptKind, contextId: ctx.id, invariants: String(data.get('invariants')).split('\n').map(x => x.trim()).filter(Boolean), position: [ctx.position[0] + Math.cos(angle) * radius, 1 + count % 3, ctx.position[2] + Math.sin(angle) * radius] }); applyMetadata(next.concepts.at(-1)!, data); selectedId = id; scopeId = ctx.id; editorOpen = true; mode = 'domain'; commit(next, 'Concept created'); scene.focus(id);
  });
...
$('#primary-action').onclick = () => { if (lens === 'meaning') contextDialog(); else workbench.inspectSelection(selectedId ?? scopeId); };
$('#keeper-action').onclick = () => { if (lens === 'governance') workbench.inspectSelection(selectedId ?? scopeId); else if (currentView().level === 'world') openIndex(); else openEditor(); };
$('#lens-meaning').onclick = () => { lens = 'meaning'; render(); };
$('#lens-governance').onclick = () => { lens = 'governance'; render(); };
...
const workSurface = () => !$('#evidence-card').hidden ? $('#evidence-card') : !$('#inspector').hidden ? $('#inspector') : null;
