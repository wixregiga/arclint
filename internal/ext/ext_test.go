...
func fakeHost(files map[string]string) ext.Host {
	var infos []ext.FileInfo
	var paths []string
	for p := range files {
	...
	return ext.Host{
		...
		Imports: func(path string) []ext.ImportInfo { return nil },
...
func TestLoadAndCheckWithDefaults(t *testing.T) {
	...
	if rt.Contract != "invariant" || rt.Blame != "provider" {
		t.Errorf("defaults: contract=%s blame=%s", rt.Contract, rt.Blame)
	}
...
`})
	_, err := ext.LoadDir(root, ext.Options{})
...
`})
	reg, err := ext.LoadDir(root, ext.Options{CheckTimeout: 200 * time.Millisecond})
	if err != nil {
		t.Fatal(err)
	}
	rt := reg.Get("spin")
...
func TestMissingDirIsEmptyRegistry(t *testing.T) {
	reg, err := ext.LoadDir(t.TempDir(), ext.Options{})
