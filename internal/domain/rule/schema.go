...
func defDescriptions() map[string]string {
	descs := map[string]string{
		...
		"override":                "An Override of a distributed or built-in Rule, keyed by that Rule's ID. It carries no constraint or rationale: it disables the Rule with a reason, changes its severity, excludes subjects, or suppresses findings. To change what a Pattern Rule asserts, disable it and add a local Rule under a new ID.",
...
func structureConstraintSchema(expanded bool) map[string]any {
	...
	if expanded {
		...
		desc = "Expanded structure: one universally quantified proposition whose globs derive from a recorded Ubiquitous Language collection. A project recording nothing derives no obligations; the Rule exists and says so."
